            ...
            if cr_code:
                ...
                if present_status:
                    if present_status[0] != "correction" or present_status[0] == "completed":
                        ...
                        if form_state_status[0] == "release":
                            print("inside")
                        ...
                        if Users.FetchUSerDetails_By_ID(decoded_token["id"])["role_id"] == 1 or form_state == "release":
                            ...
                            if onboard["status"] != "Error":
                                Cr_info.Add_cr_info(dept, yard_no, initiated_by, unit, po_number,
                                                ...
                                                datetime.now(), 1 if form_status == "rejected" else 0, Cr_info.Fetch_cr_info(cr_code)[0]["last_updated_by"] if form_state != "release" else init_by,
...
class CRForminit(MethodView):
    ...
    def post(self):
        try:
            ...
            fetchQuery = f"SELECT DISTINCT ON (unique_id) unique_id, * FROM cr_info WHERE init_by = '{_init_by}' ORDER BY unique_id, created_at DESC LIMIT 100"
            data = db.session.execute(fetchQuery)
            CrInfoSchema = cr_info_schema(many=True)
            summitted_form = CrInfoSchema.dump(data)
            ...
            fetchQuery = f"SELECT DISTINCT ON (unique_id) unique_id, * FROM cr_info ORDER BY unique_id, created_at DESC LIMIT 100"
            data = db.session.execute(fetchQuery)
            CrInfoSchema = cr_info_schema(many=True)
...
class user_heading(MethodView):
    ...
    def post(self):
        try:
            search_query = f"SELECT * FROM cr_info"
