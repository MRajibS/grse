...
class fn_Create_Terminals(MethodView):
    ...
    def post(self):
        try:
            request_data = request.get_json(force=True)
            query = Terminals.query.filter(
                (Terminals.name == request_data["name"]) | (Terminals.short_code == request_data["short_code"]) | (
                            Terminals.alpeta_terminal_id == request_data["alpeta_terminal_id"])).first()
            if query:
                respone = {"status": 'error',
                           "message": "Terminals Name or Short Code or Alpeta Terminal ID already exists"}
                return make_response(jsonify(respone)), 200
