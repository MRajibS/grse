        ...
        try:
            ...
            if search_status:
                if len(param_sort_code) != 0:
                    ...
                    search_query_p = f"SELECT * FROM terminals WHERE status != 'delete' AND short_code ilike '%{param_sort_code}%'  limit 10 offset {offset}"
                    data_p = db.session.execute(search_query_p)
                    TerminalSchema = Terminals_schema(many=True)
                    output_p = TerminalSchema.dump(data_p)
                    final_data_list = []
                    for i in output_p:
                        i['user_Count'] = len(User_terminals.Get_USers_by_TerminalID(i['id']))
                        i["status"] = "active" if i["alpeta_terminal_id"] in terminal_status(i["alpeta_terminal_id"])[
                            "online"] \
                            else "inactive"
                        final_data_list.append(i)
                    respone = {
                        ...
                        "message": "Success",
                        "count": len(output),
                        "terminals": final_data_list}
                    return make_response(jsonify(respone)), 200
                if len(param_name) != 0:
                    # "SELECT * FROM users WHERE users.status != 'delete' AND users.role_id = 3 AND users.full_name
                    # ilike '%dilip%'" search_query = Terminals.query.filter(Terminals.status != 'delete',
                    # Terminals.name.like(name)).all()
                    search_query = f"SELECT * FROM terminals WHERE status != 'delete' AND name ilike '%{param_name}%'"
                    data = db.session.execute(search_query)
                    TerminalSchema = Terminals_schema(many=True)
                    output = TerminalSchema.dump(data)
                    search_query_p = f"SELECT * FROM terminals WHERE status != 'delete' AND name ilike '%{param_name}%' limit 10 offset {offset} "
                    data_p = db.session.execute(search_query_p)
                    TerminalSchema = Terminals_schema(many=True)
                    output_p = TerminalSchema.dump(data_p)
                    final_data_list = []
                    ...
                    respone = {
                        ...
                        "terminals": final_data_list}
                    return make_response(jsonify(respone)), 200
            elif not search_status:
                All_data = Terminals.query.order_by(asc(Terminals.name)).all()
                terminalsSchema = Terminals_schema(many=True)
                output = terminalsSchema.dump(All_data)
                search_query_p = f"SELECT * FROM terminals WHERE status != 'delete' limit 10 offset {offset} "
                All_data_p = db.session.execute(search_query_p)
                terminalsSchema = Terminals_schema(many=True)
                output_p = terminalsSchema.dump(All_data_p)
                final_data_list = []
                print("search_status",offset)
                for i in output_p:
                    i['user_Count'] = len(User_terminals.Get_USers_by_TerminalID(i['id']))
                    i["status"] = "active" if i["alpeta_terminal_id"] in terminal_status(i["alpeta_terminal_id"])[
                        "online"] \
                    ...
                    final_data_list.append(i)
                respone = {"status": 'success', "message": "Success", "count": len(output),
                           "terminals": final_data_list}
                return make_response(jsonify(respone)), 200
            else:
                respone = {"status": 'error', "message": "key 'search_status' must be True or False"}
                return make_response(jsonify(respone)), 200
        except Exception as e:
            response = {"status": 'error', "message": f'{str(e)}'}
            return make_response(jsonify(response)), 200
...
#                 terminalsSchema = Terminals_schema(many=True)
#                 output = terminalsSchema.dump(All_data)
#                 print(len(output),"here")
#                 All_data_p = Terminals.query.order_by(asc(Terminals.name)).limit(10).offset(int(page)*10)
#                 terminalsSchema = Terminals_schema(many=True)
#                 output_p = terminalsSchema.dump(All_data_p)
#                 final_data_list = []
#                 for i in output_p:
...
class fn_Create_Terminals(MethodView):
    ...
    def post(self):
        try:
            request_data = request.get_json(force=True)
            query = Terminals.query.filter((Terminals.name==request_data["name"]) | (Terminals.short_code==request_data["short_code"]) | (Terminals.alpeta_terminal_id==request_data["alpeta_terminal_id"])).first()
            if query:
                respone = {"status": 'error', "message": "Terminals Name or Short Code or Alpeta Terminal ID already exists"}
                return make_response(jsonify(respone)), 200
