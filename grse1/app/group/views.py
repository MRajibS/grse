    ...
    def post(self):
        try:
            ...
            if search:
                ...
                All_data = Group.query.filter(Group.status != 'delete',Group.name.ilike(f'%{search}%') ).order_by(Group.name).all()
                GroupSchema = Group_schema(many=True)
                output = GroupSchema.dump(All_data)
                # search_query_p = f"SELECT * FROM  WHERE status != 'delete' AND name ilike '%te%' limit 10 offset {offset} "
                # All_data_p = db.session.execute(search_query_p)
                All_data_p = Group.query.filter(Group.status != 'delete',Group.name.ilike(f'%{search}%')).order_by(Group.name).limit(10).offset(offset)
                GroupSchema = Group_schema(many=True)
                ...
                # All_data_p = db.session.execute(search_query_p)
                All_data_p = Group.query.filter(Group.status != 'delete').order_by(Group.name).limit(10).offset(offset)
                GroupSchema = Group_schema(many=True)
                output_p = GroupSchema.dump(All_data_p)
            ...
            respone = {"status": 'success', "message": "Success", "count": len(output), "group": final_data_list}
            return make_response(jsonify(respone)), 200
        except Exception as e:
            response = {"status": 'error', "message": f'{str(e)}'}
            return make_response(jsonify(response)), 200
...
Group_View.add_url_rule('/all_groups_page', view_func=fn_View_Groups_Paginate, methods=['POST'])
