...
export class AdminUserViewComponent implements OnInit {
  ...
  userId: any;
  user_details: any;
  user_alpeta_details: any;
  fingerprint_selected: any;
  alpeta_fingerprint_temp: any = null;
  alpeta_face_data: any;
  alpeta_face_editdata: any = null;
  alpeta_rfidcard_data: any;
  apleta_card_editdata: any = null;
  userRawAuthlogDetails: any = null;

  terminalMaster: any[];
  groupMaster: any[];
  assignedTerminals: any[];
  alpeta_fingerprint_data: any[];
  fingerMaster: any[] = [];
  editFingerprintData: any[] = [];
  authenticationModeMaster: any[] = [];
  authenticationCombinationMaster: any[] = [];
  userAuthenticationCombination: any[] = [];
  userRawAuthLogs: any[] = [];
  rawAuthLogAttendanceFilterMaster: any[] = [];

  scanFingerprintForm: FormGroup;
  scanFacedataForm: FormGroup;
  scanCardForm: FormGroup;
  alpetaPasswordForm: FormGroup;
  dashboardPasswordForm: FormGroup;
  terminalAssignForm: FormGroup;
  profPicForm: FormGroup;
  authenticationCombinationForm: FormGroup;
  userRawAuthLogsFilterSearchForm: FormGroup;

  fromPanel: string = "";

  userRawAuthLogsTableOptions: TableOptions = {
    'page': 1,
  ...
  constructor(
    ...
    this.terminalAssignForm = formBuilder.group({
      "terminal_id": [null, Validators.compose([])],
      "group_id": [null, Validators.compose([Validators.required])],
    });

    this.profPicForm = formBuilder.group({
      "profile_picture": [null, Validators.compose([Validators.required])],
      "profile_picture_source": [null, Validators.compose([Validators.required])],
    });
    ...
    this.alpetaPasswordForm = formBuilder.group({
      "password": ['11111111', Validators.compose([Validators.required, Validators.pattern("^[0-9]*$"), Validators.minLength(8), Validators.maxLength(10)])],
    });

    this.dashboardPasswordForm = formBuilder.group({
      "password": ['11111111', Validators.compose([Validators.required, Validators.pattern("^[0-9]*$"), Validators.minLength(8), Validators.maxLength(10)])],
    });
    ...
    this.userRawAuthLogsFilterSearchForm = formBuilder.group({
    ...
    this.terminalMaster = [];
    this.groupMaster = [];
    this.assignedTerminals = [];
    this.alpeta_fingerprint_data = [];

    this.fingerprint_selected = null;
    this.alpeta_face_data = null;
    this.alpeta_rfidcard_data = null;

    this.rawAuthLogAttendanceFilterMaster = [
      { 'value': "P10", 'description': "P10" },
      { 'value': "P20", 'description': "P20" },
    ]

    this.fingerMaster = [
      { "alpeta_value": 1, "id": 1, "description": "Right Thumb" },
      { "alpeta_value": 2, "id": 2, "description": "Right Index Finger" },
      { "alpeta_value": 3, "id": 3, "description": "Right Middle Finger" },
      { "alpeta_value": 4, "id": 4, "description": "Right Ring Finger" },
      { "alpeta_value": 5, "id": 5, "description": "Right Little Finger" },
      { "alpeta_value": 6, "id": 6, "description": "Left Thumb" },
      { "alpeta_value": 7, "id": 7, "description": "Left Index Finger" },
      { "alpeta_value": 8, "id": 8, "description": "Left Middle Finger" },
      { "alpeta_value": 9, "id": 9, "description": "Left Ring Finger" },
      { "alpeta_value": 10, "id": 10, "description": "Left Little Finger" }
    ]
  ...
  async ngOnInit() {
    setTimeout(() => {
      if (this.fromPanel == 'user')
        this.UserLayoutComponent.loadPageTitle("Manage " + this.PageType.name, "", [
          { 'url': '/user/' + this.PageType.slug, 'name': this.PageType.name },
    ...
    this.activatedRoute.params.subscribe(
      params => this.userId = params['user_id']
    )
  ...
  fetchUserDetails() {
    return new Promise((resolve, reject) => {
      this.spinner.show();
      this.adminService.viewUserDetails(this.userId)
        .subscribe(res => {
          if (res.status == 'success') {
            this.user_details = res.user_data?.UserDetails;
            this.alpeta_fingerprint_data = res.user_data?.FingerPrintDetails ?? [];
            this.assignedTerminals = res.user_data?.TerminalDetails ?? [];
            this.alpeta_face_data = res.user_data?.FaceData ?? [];
            this.alpeta_rfidcard_data = res.user_data?.CardData ?? [];

            this.user_details.designation = res.user_data?.DEsignationDetails ?? null;
            this.user_details.department = res.user_data?.DepartmentDetails ?? null;
            this.user_details.shift = res.user_data?.ShiftDetails ?? null;
            this.user_details.group_info = res.user_data?.Group_info ?? null;

            this.resetUserRawAuthLogFilterForm();

            // console.log(this.assignedTerminals)

            // this.user_alpeta_details = res.data.alpeta_user_details;

            // this.terminalMaster = [];
            // res.data.master.terminals.forEach((element: any) => {
  ...
  async openAssignGroupModal() {
    Global.resetForm(this.terminalAssignForm);

    if (this.user_details?.group_info) {
      this.terminalAssignForm.patchValue({
        'group_id': this.groupMaster.find((obj: any) => {
          return obj.id == this.user_details?.group_info?.id ?? null
        }) ?? null
      })
    }

    let jobs = await this.fetchAssignUserToGroupJobs();
    if (Array.isArray(jobs) && jobs.length > 0) {
      let userjob = jobs.find((obj: any) => {
        return obj.alpeta_user_id == this.user_details.alpeta_user_id
      })

      if (userjob) {
        this.toastr.error("A job is currently running for \"" + this.user_details.full_name + "\". Please try again after sometime");
        return;
      }
    }

    $('#open-assign-terminal-modal').click();
  }

  async fetchAssignUserToGroupJobs() {
    return new Promise((resolve, reject) => {
      this.spinner.show();
  ...
  fetchTerminals() {
    return new Promise((resolve, reject) => {
      this.spinner.show();
      this.adminService.fetchAlpetaTerminals()
        .subscribe(res => {
          this.spinner.hide();
          if (res.status == 'success') {
            this.terminalMaster = [];
            res.treminals.TerminalList.forEach((element: any) => {
              this.terminalMaster.push({
                'id': element.ID,
                'description': element.Name,
              })
  ...
  async assignTerminal(event: any) {
    this.terminalAssignForm.markAllAsTouched();
    setTimeout(function () {
    ...
    // let jobs = await this.fetchAssignUserToGroupJobs();
    // if (Array.isArray(jobs) && jobs.length > 0) {
    //   this.toastr.error("A job is currently running. Please try again after sometime");
    //   return;
    // }
    ...
    if (this.terminalAssignForm.valid) {
      event.target.classList.add('btn-loading');

      this.adminService.assignUserTerminal({
        'alpeta_user_id': this.user_details.alpeta_user_id,
        'user_id': this.user_details.id,
        'group_id': this.terminalAssignForm.value.group_id?.id ?? "",
        'status': 'active',
      }).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message == 'success' ? "User assigned to terminal" : res.message);
          this.fetchUserDetails();
          $('[data-bs-dismiss="modal"]')?.click()

          this.terminalAssignForm.reset();
          for (const key in this.terminalAssignForm.controls) {
            if (Object.prototype.hasOwnProperty.call(this.terminalAssignForm.controls, key)) {
              const element = this.terminalAssignForm.controls[key];
  ...
  removeUserFromTerminal(item: any) {
    this.spinner.show();
    this.adminService.removeUserFromTerminal({
      'id': item.id,
      'user_id': this.userId,
    }).subscribe(res => {
      this.spinner.hide();
  ...
  onImageFileChanged(event: any, imagename: any, imagename_source: any, img_prev_div: any = "none") {
    if (event.target.files.length > 0) {
      const file = event.target.files[0];
      let reader = new FileReader();
      reader.readAsDataURL(file);
      reader.onload = (e: any) => {
        this.profPicForm.patchValue({
          [imagename_source]: e.target.result
        });

        if (img_prev_div != "none") {
          $('#' + img_prev_div).show();
          $('#' + img_prev_div).find('img').attr("src", e.target.result)
        }
      }
    } else {
      if (img_prev_div != "none") {
  ...
  uploadProfPic(event: any) {
    this.profPicForm.markAllAsTouched();
    setTimeout(function () {
      Global.scrollToQuery(".form-control.is-invalid.ng-invalid")
    }, 100);

    if (this.profPicForm.valid) {
      console.log()

      event.target.classList.add('btn-loading');

      this.adminService.updateProfilePic({
        'user_id': this.user_details?.id,
        'alpeta_id': this.user_details?.alpeta_user_id,
        'img_str': this.profPicForm.value.profile_picture_source,
        'type': this.profPicForm.value.profile_picture_source ? this.profPicForm.value.profile_picture_source.substring("data:image/".length, this.profPicForm.value.profile_picture_source.indexOf(";base64")) : "",
      }).subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
          this.profPicForm.reset();
          this.fetchUserDetails();
          $('[data-bs-dismiss="modal"]')?.click()
        } else if (res.status == 'val_error') {
          this.toastr.error(Global.showValidationMessage(res.errors));
  ...
  changeUserTerminalStatus(item: any) {
    this.spinner.show();
    this.adminService.changeUserTerminalStatus({
      'id': item.user_terminal_id,
      'block_from': this.datePipe.transform(Date.now(), 'YYYY-MM-dd HH:mm:ss'),
      'block_to': "2099-01-01 00:00:00",
      'terminal_id': item.alpeta_terminal_id.toString(),
      'user_id': this.user_details.id.toString(),
      'alpeta_id': this.user_details.alpeta_user_id,
      'status': (item.is_block == 0) ? 'inactive' : 'active',
    }).subscribe(res => {
      this.spinner.hide();
      this.fetchUserDetails();
      if (res.status == 'success') {
  ...
  isUserTerminalStatusEditable(item: any) {
    return $(`#terminal-status-${item.alpeta_terminal_id}-id`).find('span.badge').hasClass('badge-success');
  }


  checkFingerCaptured(finger_id: any, edit_action: any = "none") {
    let fp_data = null;

    if (edit_action == 'editing') {
      fp_data = this.editFingerprintData.find((obj: any) => {
        return obj.FingerID == finger_id ?? null
      })
    } else {
      fp_data = this.alpeta_fingerprint_data.find((obj: any) => {
        return obj.fingerid == finger_id ?? null
      })
    }
  ...
  fingerSelected(finger_id: any, edit_action: any = "none") {
    if (this.checkFingerCaptured(finger_id) && edit_action == "none") {
      let fp_data = this.alpeta_fingerprint_data.find((obj: any) => {
        return obj.fingerid == finger_id ?? null
      }) ?? null

      this.fingerprint_selected = fp_data;
    } else if (!this.checkFingerCaptured(finger_id, 'editing') && edit_action == 'editing') {
      ...
      swal.fire({
        ...
        if (result.value) {
          this.editFingerprintData = this.editFingerprintData.filter(function (value, index, arr) {
  ...
  startFingerPrintEdit() {
    this.editFingerprintData = [];
    this.alpeta_fingerprint_temp = null;
    this.alpeta_fingerprint_data.forEach(element => {
      this.editFingerprintData.push({
        'ConvImage1': element.convimage1,
        'ConvImage2': element.convimage2,
        'FingerID': element.fingerid,
        'Template1': element.template1,
        'Template2': element.template2,
        'TotalSize': element.totalsize,
        'UserID': element.alpeta_user_id,
      })
    });

    console.log(this.editFingerprintData);
    $('#open-editfingerprint-modal').click()
  }

  scanFingerPrintForEdit(event: any) {
    ...
    if (this.scanFingerprintForm.valid) {
      ...
      this.adminService.scanUserFingerprint({
        ...
        //     "TotalSize": 74752,
        //     "UserID": this.user_details?.alpeta_user_id
        //   },
        ...
        if (res.status == 'success') {
          ...
          this.editFingerprintData.push(res.dmFPImage);
  ...
  startFaceDataEdit() {
    Global.resetForm(this.scanFacedataForm);
    this.alpeta_face_editdata = null;
    $('#open-editfacedata-modal').click()
  }

  scanFaceDataForEdit(event: any) {
    this.alpeta_face_editdata = null;
    ...
    if (this.scanFacedataForm.valid) {
      ...
      this.adminService.scanUserFacedata({
        ...
        if (res.status == 'success') {
          this.toastr.success(res.message);
          this.alpeta_face_editdata = res.UserFaceWTInfo;
        } else if (res.status == 'val_error') {
          this.toastr.error(Global.showValidationMessage(res.errors));
        } else {
          this.toastr.error(res.message);
  ...
  startCardDataEdit() {
    Global.resetForm(this.scanCardForm)
    this.apleta_card_editdata = null;
    $('#open-editcarddata-modal').click()
  }

  scanCardForEdit(event: any) {
    this.apleta_card_editdata = null;
    ...
    if (this.scanCardForm.valid) {
      ...
      this.adminService.scanUserCard({
        ...
        if (res.status == 'success') {
          this.toastr.success(res.message);
          this.apleta_card_editdata = res.CardData;
        } else if (res.status == 'val_error') {
          this.toastr.error(Global.showValidationMessage(res.errors));
        } else {
          this.toastr.error(res.message);
  ...
  openAlpetaPasswordEditModal() {
    Global.resetForm(this.alpetaPasswordForm);
    $('#open-editalpetapassword-modal').click();
  }

  openDashboardPasswordEditModal() {
    Global.resetForm(this.dashboardPasswordForm);
    $('#open-editdashboardpassword-modal').click();
  }

  openAuthCombEditModal() {
    Global.resetForm(this.authenticationCombinationForm);
    this.initAuthComb()

    if (this.user_details.auth_comb) {
      let updated_authcomb = JSON.parse(this.user_details.auth_comb);

      this.authenticationCombinationForm.patchValue({
        "authentication_mode": this.authenticationModeMaster.find((obj: any) => {
          return obj.value == updated_authcomb?.authentication_mode ?? null
        }) ?? null,
      })

      this.userAuthenticationCombination = updated_authcomb.authentication_combination;

      let combLeft: any[] = [];
      this.authenticationCombinationMaster.forEach(element => {
        let temp = this.userAuthenticationCombination.find((obj: any) => {
          return obj.value == element.value
        })

        if (!temp) {
          combLeft.push(element)
        }
      });

      this.authenticationCombinationMaster = combLeft;
    }

    $('#open-editauthcombination-modal').click();
  }
  ...
  updateUser(event: any, type: any) {
    let parameters: any = {
      'type': type,
      'user_id': this.user_details.id,
      'alpeta_id': this.user_details.alpeta_user_id.toString(),

      'alpeta_fingerprint_data': null,
      'alpeta_face_data': null,
      'cardnum': null,
      'auth_comb': null,
      'password': null,
    };
    ...
    switch (type) {
      case 'finger':
        if (this.editFingerprintData.length < 1) {
          this.toastr.error("You must select atleast one fingerprint before updating");
          return;
        }

        parameters.alpeta_fingerprint_data = this.editFingerprintData;
        break;

      case 'face':
        if (!this.alpeta_face_editdata) {
          this.toastr.error("You must scan new facedata before updating");
          return;
        }

        parameters.alpeta_face_data = this.alpeta_face_editdata;
        break;

      case 'card':
        if (!this.apleta_card_editdata) {
          this.toastr.error("You must scan new card data before updating");
          return;
        }

        parameters.cardnum = this.apleta_card_editdata?.CardNum ?? "";
        break;

      case 'password':
        this.alpetaPasswordForm.markAllAsTouched();
        setTimeout(function () {
          Global.scrollToQuery(".form-control.is-invalid.ng-invalid")
        }, 100);

        if (!this.alpetaPasswordForm.valid) {
          return;
        }

        parameters.password = this.alpetaPasswordForm.value.password;
        break;
    ...
    event.target.classList.add('btn-loading');
    this.adminService.updateUser(parameters)
      .subscribe(res => {
        if (res.status == 'success') {
          this.toastr.success(res.message);
          this.fetchUserDetails();
  ...
  updateUserDashboardPassword(event: any) {
    this.dashboardPasswordForm.markAllAsTouched();
    setTimeout(function () {
      Global.scrollToQuery(".form-control.is-invalid.ng-invalid")
    }, 100);

    if (this.dashboardPasswordForm.valid) {
      event.target.classList.add('btn-loading');

      this.adminService.updateUserDashboardPassword({
        'user_id': this.userId,
        'password': this.dashboardPasswordForm.value.password ?? ""
      }).subscribe(res => {
        if (res.status == 'success') {
          $('#editdashboardpassword-modal')?.find('[data-bs-dismiss="modal"]').click();
          this.toastr.success(res.message);
        } else if (res.status == 'val_error') {
          this.toastr.error(Global.showValidationMessage(res.errors));
        } else {
  ...
  fetchUserRawAuthLogs({ page = <any>null, xlx_export = <Boolean>false } = {}) {
    if (!this.user_details?.alpeta_user_id) {
      return;
    }

    if (!this.userRawAuthLogsFilterSearchForm.valid) {
      this.userRawAuthLogsFilterSearchForm.markAllAsTouched();
      setTimeout(function () {
        $('#open-user-rawauth-filter-modal')?.click();
      }, 500);
      return;
    } else {
      setTimeout(function () {
        $('#user-rawauth-filter-modal')?.find('[data-bs-dismiss="modal"]').click();
      }, 500);
    }

    if (page != null) {
      this.userRawAuthLogsTableOptions.page = page;
    }

    if (xlx_export) {
      this.spinner.show();
    } else {
      $('#raw-auth-logs').find('.table-processing').show()
    }

    let params: any = this.getRawAuthLogFilterParams();
    params.pageno = xlx_export == true ? 1 : this.userRawAuthLogsTableOptions.page;
    params.export = xlx_export;

    this.adminService.fetchRawAuthLogsForExport(params).subscribe(res => {
      if (xlx_export) {
        if (res.status == 'Success') {
          let authlogs: any[] = [];
        ...
        if (res.status == 'Success') {
          this.userRawAuthLogs = [];
          if (res.Search && Array.isArray(res.Search)) {
            this.userRawAuthLogs = res.Search;
          }

          /** CALCULATING THE PAGINATION NUMBERS */
          let totalDocs = res.All_Search_count ?? 0;
          let totalPages = Math.ceil(totalDocs / Global.TableLength);
          this.userRawAuthLogsTableOptions = {
            page: this.userRawAuthLogsTableOptions.page,
            limit: Global.TableLength,
            pagingCounter: (this.userRawAuthLogsTableOptions.page - 1) * Global.TableLength,
            totalDocs: totalDocs,
            totalPages: totalPages,
            hasNextPage: (this.userRawAuthLogsTableOptions.page < totalPages),
            hasPrevPage: (this.userRawAuthLogsTableOptions.page > 1),
            nextPage: (this.userRawAuthLogsTableOptions.page + 1 < totalPages) ? this.userRawAuthLogsTableOptions.page + 1 : null,
            prevPage: (this.userRawAuthLogsTableOptions.page > 1) ? this.userRawAuthLogsTableOptions.page - 1 : null
          }
          /** CALCULATING THE PAGINATION NUMBERS */
        } else if (res.status == 'val_error') {
          this.userRawAuthLogs = [];
        ...
        $('#raw-auth-logs').find('.table-processing').hide()
      }
    }, (err) => {
      this.userRawAuthLogs = [];
      this.toastr.error(Global.showServerErrorMessage(err));
  ...
  getRawAuthLogFilterParams() {
    ...
    if (this.userRawAuthLogsFilterSearchForm.value.start_date) {
      let start = moment(this.userRawAuthLogsFilterSearchForm.value.start_date, 'DD-MM-YYYY hh:mm A');
    ...
    if (this.userRawAuthLogsFilterSearchForm.value.end_date) {
      let end = moment(this.userRawAuthLogsFilterSearchForm.value.end_date, 'DD-MM-YYYY hh:mm A');
    ...
    if (this.user_details?.alpeta_user_id) {
      employee_id = this.user_details?.alpeta_user_id.toString();
      search_status = true;
    }

    if (this.userRawAuthLogsFilterSearchForm.value.attendance && Array.isArray(this.userRawAuthLogsFilterSearchForm.value.attendance) && this.userRawAuthLogsFilterSearchForm.value.attendance.length > 0) {
      search_status = true;

      attendance = [];
      this.userRawAuthLogsFilterSearchForm.value.attendance.forEach((element: any) => {
  ...
  resetUserRawAuthLogFilterForm() {
    Global.resetForm(this.userRawAuthLogsFilterSearchForm);

    this.userRawAuthLogsFilterSearchForm.patchValue({
    ...
    this.fetchUserRawAuthLogs({ page: 1 })
  }

  openUserRawAuthFilterModal() {
    $('#open-user-rawauth-filter-modal')?.click();
  }

  fetchUserRawAuthDetails(item: any) {
    this.spinner.show();
    this.adminService.fetchRawAuthLogDetails({
      'IndexKey': item?.IndexKey ?? "",
      'UniqueID': item?.PERNR ?? "",
    }).subscribe(res => {
      if (res?.status == "Success") {
        $('#open-user-rawauthlog-details-modal')?.click();
        item.details = res?.Auth_list ?? null;
        this.userRawAuthlogDetails = item;
      } else {
        this.userRawAuthlogDetails = null;
        this.toastr.error(res.message);
      }
